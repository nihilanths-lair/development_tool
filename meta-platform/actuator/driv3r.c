...
{\
    ...
    printf("\n _______________/ Служебные ячейки памяти \\_______________");\
    putchar('\n');\
    for (int i = 0; i < 5; i++)\
    {\
        printf("\n  m64[%d] = %-20llu | %02X %02X %02X %02X %02X %02X %02X %02X",\
         i, (uint64_t) m64[i],\
         (uint8_t)(m64[i]>>56),\
         (uint8_t)(m64[i]>>48),\
         (uint8_t)(m64[i]>>40),\
...
char driv3r()
{
    ...
    uint8_t m8[0x10000] = {0}; // для хранения строк
    uint64_t m64[0x10000] = {0}; // для хранения чисел
    uint8_t pc = 0;
